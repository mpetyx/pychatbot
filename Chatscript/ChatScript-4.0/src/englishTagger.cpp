		...
		if (diff > 3) // 5 word idioms
		{
			...
			else if (D->properties & (PREPOSITION|ADVERB_NORMAL|CONJUNCTION_SUBORDINATE|CONJUNCTION_COORDINATE) || D->systemFlags & CONDITIONAL_ADVERB_IDIOM)
			{
				if (!(D->systemFlags & CONDITIONAL_ADVERB_IDIOM))
				{
					...
					else if (D->properties & CONJUNCTION && !(posValues[i+1] & SIGNS_OF_NOUN_BITS) &&  !(posValues[i+2] & SIGNS_OF_NOUN_BITS) ) posValues[i] = ADVERB_NORMAL;
		...
		if (!tagged && diff > 2) // 4 word idioms
		{
			...
			else if (D->properties & (PREPOSITION|ADVERB_NORMAL|CONJUNCTION_SUBORDINATE|CONJUNCTION_COORDINATE) || D->systemFlags & CONDITIONAL_ADVERB_IDIOM)
			{
				if (!(D->systemFlags & CONDITIONAL_ADVERB_IDIOM))	
				{
					...
					else if (D->properties & CONJUNCTION && !(posValues[i+1] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) &&  !(posValues[i+2] & (NOUN_BITS|PRONOUN_BITS|DETERMINER_BITS|ADJECTIVE_BITS)) ) posValues[i] = ADVERB_NORMAL;
...
static bool FinishSentenceAdjust(bool resolved,bool & changed,unsigned int start,unsigned int end)
{
	...
	if (!mainVerb && !assignedMainSubject && posValues[startSentence] & CONJUNCTION && allOriginalWordBits[startSentence] & ADVERB_BITS)
	{
...
bool IsDeterminedNoun(unsigned int i,unsigned int& det)
{
	...
	det = i - 1;
	while (det > 1 && posValues[det] & (ADJECTIVE_BITS|ADVERB_BITS) && !(posValues[det] & (ADJECTIVE_NUMBER|NOUN_PROPER_SINGULAR|NOUN_PROPER_PLURAL|COMMA|CONJUNCTION))) 
	{
		--det;
	}
...
static unsigned int GuessAmbiguousAdjective(unsigned int i, bool &changed)
{
	...
	if (needRoles[roleIndex] & SUBJECT_COMPLEMENT) 
	{
		...
		if (posValues[i] == (ADJECTIVE_NORMAL|ADVERB_BITS)  && (posValues[i+1] & (CONJUNCTION|COMMA|PAREN) || i == endSentence))
		{
...
//		if (LimitValues(i,-1 ^ ADJECTIVE_BITS,"possible adjective is followed by a pronoun, so no.",changed)) return GUESS_RETRY; // might not be real -
	...
	// if ambiguity is Adjective_basic Adverb_basic and wanting adjective as object, be adjective if next is junk "Samson was powerful but foolish"  but not "Max looked *all_over for her" and not "the ball was *inside"
	if (posValues[i] == (ADJECTIVE_NORMAL|ADVERB_BITS) && needRoles[roleIndex] & SUBJECT_COMPLEMENT && (posValues[i+1] & (CONJUNCTION|COMMA|PAREN) || i == endSentence) &&
...
{	
	...
	if (posValues[i-1] & (VERB_TENSES|NOUN_GERUND|NOUN_INFINITIVE) && posValues[i+1] & CONJUNCTION)
	{
...
static unsigned int GuessAmbiguousAux(unsigned int i, bool &changed)
{
	...
	if (roleIndex == MAINLEVEL && roleIndex == MAINLEVEL && !(needRoles[roleIndex] & (VERB2|MAINVERB)) && phrases[i-1])
	{
	 ...
	 if (posValues[i] & VERB_TENSES && posValues[i] & AUX_VERB_BITS && posValues[i+1] & (DETERMINER|ADJECTIVE_BITS|NOUN_BITS) && !(posValues[i+1] & CONJUNCTION)) // but not "*do as I say"
	 {
